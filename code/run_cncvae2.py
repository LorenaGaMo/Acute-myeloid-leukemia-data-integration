...
from models.cncvae2 import CNCVAE
...
if (args.fold == '0'): # whole data set
    ...
    if (args.integration == 'exp+meth'): #exp+meth
        s1_train = normalizeRNA(dataset.train['expnp']) 
        s2_train = normalizeRNA(dataset.train['methnp'])
        ...
        s1_train = normalizeRNA(dataset.train['methnp']) #integrate meth+mirna)
        s2_train = normalizeRNA(dataset.train['mirnanp'])
